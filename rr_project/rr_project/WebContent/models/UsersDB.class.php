...
class UsersDB {
	...
	public static function addUser($user, $userData) {
		// Inserts the User object $user into the Users table and returns userId
		$query = "INSERT INTO Users (userName, password, picture, firstName, lastName, address, neighborhood, dateOfBirth, gender, email, phone, url)
		                      VALUES(:userName, :password, :picture, :firstName, :lastName, :address, :neighborhood, :dateOfBirth, :gender, :email, :phone, :url)";
		try {
			...
			$statement->bindValue(":picture", $userData->getPicture());
			$statement->bindValue(":firstName", $userData->getFirstName());
			$statement->bindValue(":lastName", $userData->getLastName());
			$statement->bindValue(":address", $userData->getAddress());
			$statement->bindValue(":neighborhood", $userData->getNeighborhood());
			$statement->bindValue(":dateOfBirth", $userData->getDateOfBirth());
			$statement->bindValue(":gender", $userData->getGender());
			//$statement->bindValue(":genres", $userData->getGenres());
